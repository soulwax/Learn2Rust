...
fn repo_root() -> Result<PathBuf, StatusError> {
    ...
    if root.is_empty() {
        return Err(StatusError::Git(
            "git rev-parse returned an empty repository root".to_string(),
        ));
    }
    Ok(PathBuf::from(root))
}

fn run_git_log(root: &Path) -> Result<CommitInfo, StatusError> {
    let output = Command::new("git")
...
fn run_cargo_metadata(root: &Path) -> Result<Vec<cargo_meta::WorkspaceMember>, StatusError> {
...
fn run_cargo_test(root: &Path, member: &str) -> Result<(u32, u32), StatusError> {
    let output = Command::new("cargo")
        .args(["test", "-p", member])
        .current_dir(root)
        .output()
...
fn read_status_md_front_matter(root: &Path) -> Result<status_md::FrontMatter, StatusError> {
    let text = std::fs::read_to_string(root.join("STATUS.md"))
...
pub fn run(out_path: &Path) -> Result<(), StatusError> {
    let root = repo_root()?;
    let output_path = if out_path.is_absolute() {
        out_path.to_path_buf()
    } else {
        root.join(out_path)
    };
    let last_commit = run_git_log(&root)?;
    let front_matter = read_status_md_front_matter(&root)?;
    let members = run_cargo_metadata(&root)?;

    let mut crates = Vec::new();
    for member in &members {
        let (passed, failed) = run_cargo_test(&root, &member.name)?;
    ...
    std::fs::write(output_path, json).map_err(|e| StatusError::Io(e.to_string()))?;
